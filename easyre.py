...
class RE(object):
    ...
    def __mul__(self, other):
        if not isinstance(other, int) or other < 0:
            raise TypeError("Can only multiple regular expressions by positive integers")
...
class Or(TwoOp):
    ...
    def optimize(self):
        ...
        if isinstance(self.left, CharacterSet) and isinstance(self.right, CharacterSet):
            return CharacterSet(self.left._characters + self.rigth._characters)

        if isinstance(self.left, CharacterRange) and isinstance(self.right, CharacterRange):
            ranges = set()
            ranges.update(self.left.ranges())
            ranges.update(self.right.range())
    ...
    def __init__(self, *ranges):
        ...
        for start, stop in ranges:
            if not isinstance(start, six.string_types) or len(start) != 1:
                raise ValueError('Start must be a single characters')
            if not isinstance(stop, six.string_types) or len(stop) != 1:
                raise ValueError('Stop must be a single characters')
...
class NumericRange(RE):
    ...
    def __new__(self, low, high, zeropad=False):
        ...
        if not zeropad:
            return WordSet(map(str, xrange(low, high)))
